...
public class LeetCodeProblems {
	...
	public void rotate(int[] nums, int k) {
		...
		/*
		 * int [] res = new int[nums.length]; if(nums.length ==1){} else{ for(int i = 0;
		 * i<nums.length;i++) { if(i<=k && ((k-i-1) !=-1) ){ res[k-i-1] =
		 * nums[nums.length - 1-i]; } else { res[i] = nums[i-k]; } } for(int i = 0;
		 * i<res.length;i++){ nums[i] = res[i]; } }
		 */
	...
	public void sortColors(int[] nums) {
		...
		/*
		 * int zeroes = 0; int ones = 0; for(int i = 0 ; i<nums.length; i++){ if(nums[i]
		 * == 0) zeroes++; else if(nums[i] ==1) ones++; }
		 * 
		 * for(int i = 0 ; i<nums.length; i++){ if(i<zeroes) nums[i] = 0; else
		 * if(i<zeroes+ones) nums[i] =1; else nums[i] = 2; }
		 */
	...
	public int[] nextGreaterElement(int[] nums, int[] nums2) {
		// worst approach
		/*
		 * boolean flag = false; Map<Integer, Integer> mapper = new HashMap<>(); int[]
		 * res = new int[nums.length]; for(int i = 0; i<nums2.length;i++){ int num =
		 * nums2[i]; int max = nums2[i];
		 * 
		 * for(int j = i+1; j<nums2.length; j++){ if(nums2[j] > max){ max = nums2[j];
		 * if(!mapper.containsKey(num)){ mapper.put(num, max); } } } } for(int i = 0;
		 * i<nums.length;i++){ if(mapper.get(nums[i])!= null){ res[i] =
		 * mapper.get(nums[i]); } else { res[i] = -1; } }
		 */
		// brute
		boolean flag = false;
	...
	public boolean isAnagram(String s, String t) {
		...
		// Approcah 2:
		/*
		 * if(s.length() != t.length()){ return false; } int[] alphabetArr = new
		 * int[26];
		 * 
		 * for(int i = 0; i<s.length();i++){ alphabetArr[s.charAt(i) - 'a']++;
		 * alphabetArr[t.charAt(i) - 'a']--; }
		 * 
		 * for(int counter: alphabetArr ){ if(counter!=0) return false; }
		 */
	...
	public int[] intersection(int[] nums, int[] nums1) {
		...
		/*
		 * public int [] intersection(int[] nums, int[] nums1) { int count = 0;
		 * Map<Integer, Integer> checker1 = new HashMap<>(); for(int i = 0; i<
		 * nums.length; i++){ for(int j = 0; j< nums1.length; j++){ if(nums[i] ==
		 * nums1[j] && !checker1.containsKey(nums[i])){ count++; checker1.put(nums[i],
		 * 1); break; } } } int[] ans = new int[count]; int index = 0; Map<Integer,
		 * Integer> checker = new HashMap<>(); for(int i = 0; i< nums.length; i++){
		 * for(int j = 0; j< nums1.length; j++){ if(nums[i] == nums1[j] &&
		 * !checker.containsKey(nums[i])){ ans[index++] = nums[i]; checker.put(nums[i],
		 * 1); break; } } } return ans;
		 */
	}
